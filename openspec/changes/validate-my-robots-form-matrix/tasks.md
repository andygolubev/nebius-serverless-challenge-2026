...
- [ ] 7.1 Inventory every user-visible `Unsupported Task`, `Unsupported Scene`, and `Optional Objects Not Supported` readiness reason emitted by the current builder, map it to its model/task/terrain/object configuration, and update the proposal, design, and delta specs with the expanded supported-training contract.
...
- [ ] 7.5 Update the My Robots UI so valid saved configurations show preparation/training actions and accurate capability/readiness copy instead of `Unsupported Task`, `Unsupported Scene`, or `Optional Objects Not Supported`; preserve stable disabled, retry, active-job, completed-job result-link/re-run confirmation, and quota behavior for genuine lifecycle states.
