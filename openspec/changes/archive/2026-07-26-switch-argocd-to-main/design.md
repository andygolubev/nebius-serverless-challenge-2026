...
`main`. The active development and temporary deployment branch is `main`.

## Goals / Non-Goals

**Goals:**

- Make every repository-managed Argo CD source revision use `main`.
...
- Rewrite history, merge `main` into `main`, or commit/push any `main` ref.

## Decisions

- **Update the root and child source revisions.** The root Application must be repointed
  manually to make the live cluster read `deploy/argocd` from `main`; child manifests
...
- **Preserve bootstrap consistency.** The OpenTofu variable default, example input, and the
  configured ignored `saas.auto.tfvars` use `main` so a future server recreation does
  not revert to `main`.
...
- **Push only after validation.** Commit and push the existing `main` branch, explicitly
  using `origin main`; no `main` ref is read as a push target or mutated.

## Risks / Trade-offs

- [A branch is absent on the remote] → Verify/push `main` before live cutover so Argo CD
  can resolve it.
...
1. Validate and push the repository change on `main` only.
2. Verify the server SSH key fingerprint out of band and repair the local trust record only after
   verification.
3. Manually update the live root Application's `spec.source.targetRevision` to `main`.
