...
**Reason**: Gallery examples are no longer trainable. The showcase is read-only evidence of runs that
were already performed, and gallery submission was the only path by which an unbounded visitor-facing
card could spend GPU budget.
**Migration**: `POST /jobs` no longer accepts `gallery_example_id`, `gallery_profile_id`, or `preset`.
A tenant who wants to train uploads a robot, builds a bounded setup, prepares it, and starts it via
`POST /robot-setups/{setup_id}/training-jobs`. Historical jobs that recorded a
`gallery_example_id` keep that identity for display.

### Requirement: Executability-gated publication
**Reason**: Publication is no longer gated on whether an entry *can be submitted*, because nothing
can be submitted. It is now gated on whether the entry's pinned curated run produced complete,
valid, readable evidence.
**Migration**: Replaced by "Evidence-gated publication" in the `public-training-showcase`
capability, which gates on manifest validity, required-artifact completeness, and recorded evaluation
state instead of on runtime image, compute shape, timeout, and acceptance-result resolution.

### Requirement: Bring Your Robot remains isolated
**Reason**: The isolation direction has inverted. This requirement existed to keep unvalidated custom
assets out of a trainable public gallery; now the public gallery is not trainable at all and custom
robot training is the only supported training path, so an isolation rule pointed at custom assets
describes the wrong risk.
**Migration**: Custom robot eligibility, preparation gating, and quota enforcement continue to be
governed by `custom-robot-training-preparation` and `saas-job-customization`. The showcase's
inability to start any training is covered by "Showcase cannot start training" in
`public-training-showcase`.
