## 1. Main image delivery

- [x] 1.1 Update the SaaS image workflow to build relevant `main` pushes and advance only that branch's GitOps image tag.
...
- [x] 2.2 Record local verification, commit the change, and push only `main`.
- [x] 2.3 Verify the GitHub Actions run publishes the immutable image and the bot GitOps commit updates `main`.

## 3. Live Argo CD cutover

- [x] 3.1 Independently verify the SaaS server's changed SSH host key and repair local trust.
- [x] 3.2 Repoint, refresh, and sync the live root Argo CD Application to `main`.
