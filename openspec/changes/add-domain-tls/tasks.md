...
- [x] 1.1 Pin the current stable cert-manager chart version and create
...
- [x] 1.2 Verify the root app-of-apps path picks up the new child Application (same
      `deploy/argocd/` directory the root app watches)

## 2. Let's Encrypt issuers

- [x] 2.1 Create `deploy/manifests/cert-manager/` kustomize directory with
...
- [x] 2.2 Add a `letsencrypt-staging` ClusterIssuer alongside it for rate-limit-safe
      troubleshooting
- [x] 2.3 Create `deploy/argocd/cert-manager-issuers-app.yaml` syncing that directory, with a
      later sync-wave than the cert-manager app and automated sync + retry so cold bootstrap
      converges after CRDs exist

## 3. SaaS ingress TLS

- [x] 3.1 Update `deploy/manifests/saas/ingress.yaml`: set
...
- [x] 3.2 Decide on and (if safe) enable the HTTP→HTTPS redirect per design Decision 4;
      otherwise record deferral in the design doc

## 4. Documentation

- [x] 4.1 Update `sim2policy/infra/nebius/README.md`: document the domain, the manual DNS A
      record prerequisite (`sim-policy-trainer-challenge.info` → server public IP), and where
      to change the domain if it moves
- [x] 4.2 Note in `saas.tfvars.example`/README that no firewall change is needed (80/443
