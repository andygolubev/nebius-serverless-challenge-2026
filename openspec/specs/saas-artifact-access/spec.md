...
## Purpose
Provide secure access to durable training artifacts in Nebius Object Storage — tenant-authorized for
an owned job, pinned-run-allowlisted for a published showcase entry — and ensure submitted training
jobs write to the same per-run S3 prefixes without exposing credentials. The archive *contents* and
integrity contract for `policy-bundle.zip` are specified in `policy-bundle-export`; this capability
owns only how any manifest-declared artifact, including that bundle, is resolved and delivered.
## Requirements
