...
A GitHub Actions workflow SHALL build the SaaS application container image and push it to the
existing Nebius Registry on merges to the default branch and on version tags. Images SHALL be tagged
with an immutable identifier (the commit SHA) in addition to any moving tag.

#### Scenario: Merge builds and pushes an image
- **WHEN** a commit is merged to the default branch
- **THEN** the workflow builds the SaaS image and pushes it to the Nebius Registry tagged with the
  commit SHA
