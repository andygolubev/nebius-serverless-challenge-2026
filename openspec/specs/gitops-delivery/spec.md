...
equivalent root `Application`) pattern, so that a merge to the tracked branch is the only action
required to change what runs on the cluster.

#### Scenario: Git is the source of truth
- **WHEN** a Kubernetes manifest for the SaaS app is changed and merged to the tracked branch
