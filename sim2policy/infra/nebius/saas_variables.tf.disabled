# Variables for the always-on SaaS control-plane server (add-saas-server change).
...
variable "saas_argocd_repo_url" {
  description = "Git repository ArgoCD syncs the root app-of-apps from (e.g. https://github.com/<org>/<repo>.git)."
  type        = string
}
...
variable "github_token" {
  description = "GitHub token ArgoCD uses to read the manifests repo. Stored only in MysteryBox; keep out of Git (use a gitignored tfvars or -var)."
  type        = string
  sensitive   = true
}

variable "saas_use_registry_pull_secret" {
  description = "When true, create a MysteryBox registry pull credential and wire an imagePullSecret. Leave false to rely on the VM service account's registry.puller IAM role (preferred)."
  type        = bool
  default     = false
}
